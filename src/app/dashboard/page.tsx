    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                throw new Error("Deletion failed: Record was not found or permission denied on server.");
            }

            setResumes(prev => prev.filter(r => r.id !== id));
        } catch (err) {
            console.error("Error deleting resume:", err);
            alert(err instanceof Error ? err.message : "Failed to delete resume. This might be due to database permissions (RLS). Please ensure you've applied the required RLS policies.");
        }
    };
        ...
        <div className="min-h-screen bg-background text-foreground font-sans">
            ...
            <main className="max-w-7xl mx-auto px-6 md:px-12 py-12">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                    ...
                    <Link
                        href="/build"
                        className="bg-primary text-black font-bold uppercase tracking-widest px-6 py-3 rounded-sm flex items-center gap-2 hover:bg-primary/90 transition-all shadow-[0_0_20px_rgba(34,197,94,0.3)] w-fit"
                    >
                        <Plus size={18} />
                        Create New Alpha
                    </Link>
                ...
                <div className="bg-card/30 backdrop-blur-md border border-border p-4 rounded-sm mb-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="relative w-full sm:max-w-md">
                        ...
                        <input
                            ...
                            className="w-full bg-background/50 border border-border pl-10 pr-4 py-2 rounded-sm font-mono text-sm focus:outline-none focus:border-primary/50 transition-colors"
                            value={searchQuery}
                    ...
                    <div className="flex items-center gap-6 font-mono text-[10px] uppercase tracking-widest text-muted-foreground whitespace-nowrap">
                        ...
                        <div className="flex items-center gap-2">
                            Status: <span className="text-green-500 font-bold">Encrypted / Online</span>
                        </div>
                    </div>
                </div>
